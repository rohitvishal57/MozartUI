import { Component } from '@angular/core';
import { FormBuilder, Validators } from '@angular/forms';
import { NgToastService } from 'ng-angular-popup';
import { AdminService } from 'src/app/services/admin.service';
import ValidateForm from 'src/app/validation/validateForm';

@Component({
  selector: 'app-agent-details',
  templateUrl: './agent-details.component.html',
  styleUrls: ['../../bank-controller/add-bank/add-bank.component.scss','../../bank-controller/add-bank/table.scss']
})
export class AgentDetailsComponent {

  visible: boolean = false;
  AgentDetailsForm:any;
  allAgency:any[]=[]
  allAgentDetails:any[]=[]
  channel:any[]=[]

  constructor(private adminService:AdminService,private toast:NgToastService,
    private fb: FormBuilder){}

  ngOnInit(){
    this.initializeForm();
    this.getAllAgency();
    this.getAllAgentDetails();
    this.getAllChannel();
  }
  initializeForm(){
    this.AgentDetailsForm=this.fb.group({
      agentUserName:['',Validators.required],
      agentMobileNum:['',Validators.required],
      agencyCode:['', Validators.required],
      agentUserStatus:[true,Validators.required],
      verticalCode:['',Validators.required]
    });
  }
  getName(id:any){
    let data=this.allAgency.find(agency => agency.agencyCode === id);
    return data;
  }
  getAllAgency() {
    return this.adminService.getAllAgencyDetails().subscribe({
      next: (res)=>{
        this.allAgency=res;
        console.log(this.allAgency);
      },
      error: (err => {
        console.error(err); 
      })
    });
  }
  getAllAgentDetails() {
    return this.adminService.getAllAgentDetailsList().subscribe({
      next: (res)=>{
        this.allAgentDetails=res;
        console.log(this.allAgentDetails);
      },
      error: (err => {
        console.error(err); 
      })
    })
  }
  onSubmit(){
    if(this.AgentDetailsForm.valid){
      console.log(this.AgentDetailsForm.value);
      this.adminService.insertAgent(this.AgentDetailsForm.value).subscribe({
        next: (res)=>{
          this.toast.success({ detail: "SUCCESS", summary: "User Added Successfully.", duration: 4000 })
          this.AgentDetailsForm.reset();
          this.getAllAgentDetails();
        },
        error: (err => {
          this.toast.warning({ detail: "WARNING", summary:"User Already Added.!", duration: 4000 });
        })
      });
    }
    else{
      ValidateForm.validateAllFormFields(this.AgentDetailsForm);
      this.toast.warning({ detail: "WARNING", summary:"Please Fill All Required Fields.!", duration: 4000 });
    }
  }
  showDialog(data:any) {
    this.initializeForm();
    this.visible = true;
    this.AgentDetailsForm.addControl('agentUserDetailsSeq',this.fb.control(data.agentUserDetailsSeq));
    this.AgentDetailsForm.addControl('agentCode',this.fb.control(data.agentCode));
    this.AgentDetailsForm.get('agentUserName')?.setValue(data.agentUserName);
    this.AgentDetailsForm.get('agentMobileNum')?.setValue(data.agentMobileNum);
    this.AgentDetailsForm.get('agencyCode')?.setValue(data.agencyCode);
    this.AgentDetailsForm.get('agentUserStatus')?.setValue(data.agentUserStatus);
    this.AgentDetailsForm.get('verticalCode')?.setValue(data.verticalCode);
    console.log(this.AgentDetailsForm.value);
  }
  onUpdate(){
    if(this.AgentDetailsForm.valid){
      this.adminService.updateAgent(this.AgentDetailsForm.value).subscribe({
        next: (res)=>{
          this.toast.success({ detail: "SUCCESS", summary: "User Updated Successfully.", duration: 4000 })
          this.getAllAgentDetails();
        },
        error: (err => {
          this.toast.warning({ detail: "WARNING", summary:"User Already Added.!", duration: 4000 });
        })
      });
      this.AgentDetailsForm.reset();
      this.AgentDetailsForm.removeControl('agentUserDetailsSeq');
      this.AgentDetailsForm.removeControl('agentCode');
      console.log(this.AgentDetailsForm.value);
      this.visible=false;
    }
    else{
      ValidateForm.validateAllFormFields(this.AgentDetailsForm);
      this.toast.warning({ detail: "WARNING", summary:"Please Fill All Required Fields.!", duration: 4000 });
    }
  }
  getAllChannel(){
    this.adminService.getAllChannelList().subscribe({
      next: (res)=>{
        this.channel=res;
      },
      error: (err => {
        console.error(err); 
      })
    });
  }
}
