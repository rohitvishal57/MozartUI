...
@Component({
  selector: 'app-bataav-leads-list',
  templateUrl: './bataav-leads-list.component.html',
  styleUrls: ['./bataav-leads-list.component.scss']
})
export class BataavLeadsListComponent {
  ...
  agentDesignation: any;
    ...
    getLeadsList() {
      ...
      this.rugService.getLeadDetailsBata(req).subscribe((response: any) => {
  ...
  async actionLead(lead: any) {
    console.log(lead);
    let proposalNumber :any ='';
    localStorage.setItem('leadId', lead.leadID);
    localStorage.setItem('agentId', lead.agentCode)
    const reqData = {
      "partnerId": lead.partnerId,
      "productId": lead.productId
    }
    this.common.Getformsequence(reqData).subscribe({
      next: async (res: any) => {
        try {
          console.log(res);
          this.formSequence = JSON.parse(res.data.formSequence);
          if(lead.agentCode == "BATAAV" || lead.agentCode == "BANKRM1234"){
            sessionStorage.setItem("formIndex", "0");
          }
          if(lead.businessType == "RUG"){
            this.router.navigate(['rug'], {
              state: { productData: reqData, formSequence: this.formSequence }
            });
          }else{
            if(lead.proposalNumber){
              proposalNumber = lead.proposalNumber;
            }else{
              proposalNumber = await this.generateProposalNumnberAndUpdateLeadInfor(lead.leadID);
            }
            const productData: any = {
              ...
              isLead : true,
              leadId : lead.leadID,
              applicableZones: lead?.applicableZones,
              isAxisFlow: true
            }
            ...
            this.formData.firstName = lead.customerName
            this.formData.lastName = lead.customerLastName
            this.formData.emailId = lead.customerEmail
            this.formData.mobileNumber = lead.customerNumber
            this.formData.memberDobProposer = lead.customerDOB
            this.formData.proposalNumber = proposalNumber
            this.formData.productName = lead.productName
  ...
  navigateToSummaryPage(lead: any) {
    console.log(lead);
