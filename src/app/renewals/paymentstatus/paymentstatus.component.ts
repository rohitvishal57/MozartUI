...
export class PaymentstatusComponent {
  ...
  initializeDynamicFormControls(dynamicFormControls: any, index: any = null, parentControl: any = null, policyLength: any = 0) {
    console.log(dynamicFormControls, index, parentControl, 'parentControl');

    let formGroup: any = this.fb.group({});
    console.log(dynamicFormControls, 'dynamicFormControls')

    dynamicFormControls.forEach((control: IDynamicControl) => {
      if (control.label === 'Sum Insured') {
                    ...
                    if (innerSubControls.coreControls && selectedControl.name == innerSubControls.name) {
                      innerSubControls.coreControls.forEach((coreControl: any) => {
                        if (innerControl.name == "addOnSumInsured") {
                          ...
                          let insuredMemberDetails = this.formData.insuredMemberDetails;
                          if (parentControl.name == 'accident') {
                            index = index + 2
                          } else {
                            index = index
                          }
                          ...
                          if (selectedControl.name === this.proposer) {
                            ...
                            if (JSON.parse(event.target.value).name != this.selectedPlan) {
                                ...
                                insuredMemberDetails.forEach((member: any) => {
                                  if (member.relation == selectedControl.name) {
                                    if (member.annualIncome != "") {   //earning                            
                                      ...
                                      const occupationValue = this.selectedOccupation ? JSON.parse(this.selectedOccupation).name : ''
  ...
  handlePolicyTypeChange(control: any, planType: string | null = null): void {
    console.log(planType, this.form, 'hello')
  ...
  assignSumInsuredValue(array: any) {
    array.forEach((subArray: any) => {
      // Find the sumInsured value in the current sub-array
      const sumInsuredValue = subArray.find((obj: any) => obj.name === 'sumInsured')?.value || 1000000;

      if (sumInsuredValue !== undefined) {
        // Update any object with name 'sumInsured' in the same sub-array
        subArray.forEach((obj: any) => {
          if (obj.name === 'sumInsured') {
            obj.value = sumInsuredValue;
            obj.disable = true;
          }
        });
      }
  ...
  resetInsuredMembers(control: any, planType: any) {
    ...
    if (planType === 'Multi Individual') {
      ...
      this.form.formSections.forEach((section: any) => {
        if (section.sectionTitle == "Insured Member Details") {
          ...
          console.log(section.formControls[0].dynamicControls, 'section.formControls[1].dynamicControls[1]')
          if (section.formControls[0].dynamicControls.length > 1) {
            this.assignSumInsuredValue(section.formControls[0].dynamicControls);
          }
        }
      });

      console.log(this.form.formSections, 'this.form.formSections')
    }
    else {
      if (control.dependentControls)
        this.changeMainFormDependentControls(control.dependentControls, false, control.name);
    ...
    this.form.formSections.forEach((section: any) => {
      if (section.sectionTitle == "Insured Member Details") {
        section.formControls.forEach((control:any) => {
          control.dynamicControls.forEach((dependentControl:any,index:number) => {
            if(dependentControl.name != "emailId" && dependentControl.name != "mobileNumber"){
              // dependentControl.disabled = true;
            }
            if(dependentControl.disabled){
              // this.dynamicFormGroup.get(control.dynamicControls[index].dependentControl.name)?.disable();
            }
          })
        })
      }
      ...
      this.yatraService.GetProposerRelationships(reqData).pipe(
        tap((res: any) => {
          ...
          if (this.isQuote || this.quickQuoteRedirect || this.isPolicyDetailsFetch || this.formData.policyNumber) {
            ...
            // Loop through the members and match with relations
            control.selectCheckboxOptions.forEach((member): any => {
              const matchedRelation: any = this.getMatchingRelation(member);
              if (matchedRelation) {
                this.logSelection(null, member, control);
                console.log(`Found match for ${member}:`, matchedRelation);
              } else {
                console.log(`No match found for ${member}`);
              }
            });
            console.log(control.selectCheckboxOptions, this.formData.insuredMemberDetails, 'hello',this.form)
            // this.formData.insuredMemberDetails.forEach((option: any) => {
            //   this.logSelection(null, option.relation, control);
  ...
  getMatchingRelation(member: any) {
    const relationValue = member.value;

    // Logic to compare relation with name in array 1 (relationsArray)
    for (const relation of this.formData.insuredMemberDetails) {
      console.log(relation, relationValue, 'relationValue')
      // Check if the relation value matches or it is a suffix-based match (like Sister1, Son1, etc.)
      if (relation.relationshipType.name.includes(relationValue)) {
        return relation;
      }
    }
  ...
  getProposerRelationships(control: IFormControl): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.yatraService.GetProposerRelationships(reqData).pipe(
        tap((res: any) => {
          ...
          console.log(control);
          console.log(this.formData, this.form, this.dynamicFormGroup);
...
logSelection(event: Event | null, option: any, controls: any) {
  ...
  async onSubmit(event?: any) {
    ...
    console.log(this.dynamicFormGroup.getRawValue(), this.dynamicFormGroup.value, this.form);
    ...
    if(this.form.formTitle == 'policy Details'){
      ...
      if(this.baseDate.proposerAddress1 != data.proposerAddress1 || this.baseDate.proposerPincode != data.proposerPincode || 
        this.baseDate.proposerAddress3 != data.proposerAddress3  || this.baseDate.proposerAddress2 != data.proposerAddress2 ){
         updateAddress = {
          proposerAddress1 : data.proposerAddress1,
          proposerPincode : data.proposerPincode,
          proposerAddress3 : data.proposerAddress3,
          proposerAddress2 : data.proposerAddress2,
      ...
      if (tempMember) {
        let mappedMembers = tempMember.map((membersInfo: any, index: number) =>
          JSON.stringify({
            ...
            relationCode : JSON.parse(membersInfo?.memberType).id || "",
            preFix: membersInfo?.preFix ?? "",
            annualIncome: membersInfo?.annualIncome ?? "",
            natureOfDuty: membersInfo?.natureOfDuty ? JSON.parse(membersInfo.natureOfDuty).name ?? "" : "",
            designation: membersInfo?.designation ? JSON.parse(membersInfo.designation).name ?? "" : "",
            occupation: membersInfo?.occupation ? JSON.parse(membersInfo.occupation).name ?? "" : "",
      ...
      // await this.renewalService.updateRenewalMemberAndAddress(leadPageRequestBody).subscribe({
      //   next: (res: any) => {
      //     console.log(res);
      //   },
      //   error: (err) => {
...
//         "Member":JSON.stringify(this.dynamicFormGroup.getRawValue().insuredMemberDetails),
// "addressDetails":JSON.stringify(
//   {
//     proposerAddress1:this.dynamicFormGroup.getRawValue().proposerAddress1,
//     proposerAddress2:this.dynamicFormGroup.getRawValue().proposerAddress2,
//     proposerAddress3:this.dynamicFormGroup.getRawValue().proposerAddress3,
//     proposerPincode:this.dynamicFormGroup.getRawValue().proposerPincode,
//     city:this.dynamicFormGroup.getRawValue().city,
//     state:this.dynamicFormGroup.getRawValue().state
//   })
//         // "partnerId": this.partnerId,
//         // "agentCode": this.agentCode,
//         // "formData": JSON.stringify(this.dynamicFormGroup.getRawValue()),
...
// console.log(reqData,this.dynamicFormGroup.getRawValue(),'reqData')
//       // await this.yatraService.InsertorupdateRenewalAddMember(reqData).subscribe({
//       //   next: (res: any) => {
...
//     }
  ...
  generateHeader(control: any, i: any) {
    console.log(this.form, control.name, 'control', i, this.formData,this.dynamicFormGroup,this.dynamicFormGroup.value)
    if (control.name) {
      const imagePath = this.formData[control.name][i - 1]?.relationshipType?.imagePath;
      return imagePath;
    }
    else {
      console.log(this.formData, control, 'hello')
      const imagePath = this.formData[control.name][i - 1].relationshipType?.imagePath || '';
      return imagePath;
    }
  }
